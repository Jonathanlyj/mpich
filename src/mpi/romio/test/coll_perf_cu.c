...
int main(int argc, char **argv)
{
    ...
    int order, nprocs, len, *buf, *d_buf, mynod, item_count;
    MPI_Count bufcount;
    int array_of_dargs[3], array_of_psizes[3];
    MPI_File fh;
    MPI_Status status;
    double offstim, stim, write_tim, off_tim = 0.0, new_write_tim, new_off_tim, write_bw;
...
/* process 0 takes the file name as a command-line argument and
   broadcasts it to other processes */
    ...
    array_of_gsizes[0] = 128 * 10; //detault is 17 but LEIA can only takes up to 10
...
/* now time write_all */
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    offstim = MPI_Wtime();
    CUDA_CHECK(cudaMemcpy(d_buf, buf, item_count * sizeof(int), cudaMemcpyHostToDevice));
    stim = MPI_Wtime();
    MPI_CHECK(MPI_File_write_all(fh, buf, item_count, MPI_INT, &status));
    write_tim = MPI_Wtime() - stim;
    off_tim += stim - offstim;
    
    MPI_CHECK(MPI_File_close(&fh));

    MPI_Allreduce(&write_tim, &new_write_tim, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
    MPI_Allreduce(&off_tim, &new_off_tim, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);

    if (mynod == 0) {
        write_bw =
            (long) (array_of_gsizes[0] * (long) array_of_gsizes[1] * (long) array_of_gsizes[2] *
                    sizeof(int)) / ((new_write_tim + new_off_tim) * 1024.0 * 1024.0);
        fprintf(stderr, "Global array size %d x %d x %d integers\n", array_of_gsizes[0],
                array_of_gsizes[1], array_of_gsizes[2]);
        fprintf(stderr,
                "Offloading time = %f sec, Collective write time = %f sec, Collective write bandwidth = %f Mbytes/sec\n",
                new_off_tim, new_write_tim, write_bw);
    }
