                                   ...
                                   ADIO_Status * status, int *error_code)
{
    ...
    if (myrank==4) printf("--------------------- ADIOI_LUSTRE_WriteStridedColl start ------------------------------\n");
    ...
    if ((fd->romio_write_aggmethod == 1) || (fd->romio_write_aggmethod == 2)) {
         ...
         */
        if (myrank==4) printf("%s before ADIOI_LUSTRE_Calc_my_req: %lf \n", __func__, MPI_Wtime() - start_time);
        start_time = MPI_Wtime();
        ...
        if (myrank==4) printf("%s after ADIOI_LUSTRE_Calc_my_req: %lf \n", __func__, MPI_Wtime() - start_time);
        start_time = MPI_Wtime();
         ...
         */
        if (myrank==4) printf("%s before ADIOI_LUSTRE_Calc_others_req: %lf \n", __func__, MPI_Wtime() - start_time);
        start_time = MPI_Wtime();
        ...
        if (myrank==4) printf("%s after ADIOI_LUSTRE_Calc_others_req: %lf \n", __func__, MPI_Wtime() - start_time);
        start_time = MPI_Wtime();
...
if (!fd->is_agg) {
    ...
    assert(my_aggr_idx >= 0);
         ...
         */
        if (myrank==4) printf("%s before ADIOI_LUSTRE_Exch_and_write: %lf \n", __func__, MPI_Wtime() - start_time);
    ...
    if (myrank==4) printf("%s function end: %lf \n", __func__, MPI_Wtime() - start_time);
    if (myrank==4) printf("--------------------- ADIOI_LUSTRE_WriteStridedColl end ------------------------------\n");
}
                                        ...
                                        int *error_code)
{
    ...
    *error_code = MPI_SUCCESS;
    ...
    if (myrank==4) printf("%s before loops for two phase: %lf \n", __func__, MPI_Wtime() - before_loop_start_time);
    if (myrank==4) printf("\n number of rounds: %d\n", ntimes);
